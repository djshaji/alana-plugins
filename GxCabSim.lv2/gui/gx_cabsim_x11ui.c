...
static void resize_event(gx_cabsimUI *ui);
...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_cabsimUI* ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const struct _LV2UI_Descriptor * descriptor,
	...
	gx_cabsimUI* ui = (gx_cabsimUI*)malloc(sizeof(gx_cabsimUI));
	...
	ui->controls[1] = (gx_controller) {{0.0, 0.0, 0.0, 7.0, 1.0}, {180, 10, 141, 141}, false,"CAB", ENUM, CAB};
	...
	ui->width = ui->init_width -140 + (140 * CONTROLS);
	...
	ui->frame = cairo_image_surface_create (CAIRO_FORMAT_ARGB32, 141, 161);
	...
	*widget = (void*)ui->win;
	...
	ui->rescale.xc = (double)ui->width/(ui->init_width-140 + (140 * CONTROLS));
...
static void cleanup(LV2UI_Handle handle) {
	gx_cabsimUI* ui = (gx_cabsimUI*)handle;
	cairo_destroy(ui->cr);
...
static void knob_expose(gx_cabsimUI *ui,gx_controller* knob) {
	...
	if (knob->type == SWITCH) {
		...
		cairo_pattern_add_color_stop_rgb (pat, 0, 0.1+0.48*knobstate, 0.1+0.48*knobstate,0.1+0.48*knobstate);
		cairo_pattern_add_color_stop_rgb (pat, 0.7, 0.2+0.28*knobstate, 0.2+0.28*knobstate, 0.2+0.28*knobstate);
		cairo_pattern_add_color_stop_rgb (pat, 1, 0.1+0.48*knobstate, 0.1+0.48*knobstate,0.1+0.48*knobstate);  
	} else {
		cairo_pattern_add_color_stop_rgb (pat, 0, 0.1, 0.1, 0.1);
		cairo_pattern_add_color_stop_rgb (pat, 0.7, 0.2, 0.2, 0.2);
		cairo_pattern_add_color_stop_rgb (pat, 1, 0.1, 0.1,0.1);
	...
	if (knob->type == KNOB && knob->is_active == true) {
		...
		cairo_set_source_rgba (ui->crf, 0.6, 0.6, 0.6,0.6);
		if(knob->adj.value == 0) cairo_set_source_rgba (ui->crf, 0.8, 0.8, 0.8,0.8);
		cairo_text_extents(ui->crf,"1", &extents);
		cairo_move_to (ui->crf, knobx1-knob_x/1.1-extents.width/1.6, knoby1+knob_y/1.8+extents.height/1.4);
		cairo_show_text(ui->crf, "Cream");
		cairo_new_path (ui->crf);

		cairo_set_source_rgba (ui->crf, 0.6, 0.6, 0.6,0.6);
		if(knob->adj.value == 1) cairo_set_source_rgba (ui->crf, 0.8, 0.8, 0.8,0.8);
		cairo_text_extents(ui->crf,"JMP", &extents);
		cairo_move_to (ui->crf, knobx1-knob_x/1.4-extents.width/1.6, knoby1+knob_y/2.4-extents.height/1.4);
		cairo_show_text(ui->crf, "JMP");
		cairo_new_path (ui->crf);

		cairo_set_source_rgba (ui->crf, 0.6, 0.6, 0.6,0.6);
		if(knob->adj.value == 2) cairo_set_source_rgba (ui->crf, 0.8, 0.8, 0.8,0.8);
		cairo_text_extents(ui->crf,"JTM", &extents);
		cairo_move_to (ui->crf, knobx1-knob_x/1.4-extents.width/1.6, knoby1-knob_y/3.4+extents.height/1.4);
		cairo_show_text(ui->crf, "JTM");
		cairo_new_path (ui->crf);

		cairo_set_source_rgba (ui->crf, 0.6, 0.6, 0.6,0.6);
		if(knob->adj.value == 3) cairo_set_source_rgba (ui->crf, 0.8, 0.8, 0.8,0.8);
		cairo_text_extents(ui->crf,"Laney", &extents);
		cairo_move_to (ui->crf, knobx1-knob_x/2.0-extents.width/1.6, knoby1-knob_y/1.4+extents.height/1.4);
		cairo_show_text(ui->crf, "Laney");
		cairo_new_path (ui->crf);

		cairo_set_source_rgba (ui->crf, 0.6, 0.6, 0.6,0.6);
		if(knob->adj.value == 4) cairo_set_source_rgba (ui->crf, 0.8, 0.8, 0.8,0.8);
		cairo_text_extents(ui->crf,"Triamp", &extents);
		cairo_move_to (ui->crf, knobx1+knob_x/1.4-extents.width/1.6, knoby1-knob_y/1.4+extents.height/1.4);
		cairo_show_text(ui->crf, "Triamp");
		cairo_new_path (ui->crf);

		cairo_set_source_rgba (ui->crf, 0.6, 0.6, 0.6,0.6);
		if(knob->adj.value == 5) cairo_set_source_rgba (ui->crf, 0.8, 0.8, 0.8,0.8);
		cairo_text_extents(ui->crf,"Triaxis", &extents);
		cairo_move_to (ui->crf, knobx1+knob_x-extents.width/1.5, knoby1-knob_y/3.6+extents.height/1.4);
		cairo_show_text(ui->crf, "Triaxis");
		cairo_new_path (ui->crf);

		cairo_set_source_rgba (ui->crf, 0.6, 0.6, 0.6,0.6);
		if(knob->adj.value == 6) cairo_set_source_rgba (ui->crf, 0.8, 0.8, 0.8,0.8);
		cairo_text_extents(ui->crf,"Twin", &extents);
		cairo_move_to (ui->crf, knobx1+knob_x-extents.width/1.1, knoby1+knob_y/2.4-extents.height/1.4);
		cairo_show_text(ui->crf, "Twin");
		cairo_new_path (ui->crf);

		cairo_set_source_rgba (ui->crf, 0.6, 0.6, 0.6,0.6);
		if(knob->adj.value == 7) cairo_set_source_rgba (ui->crf, 0.8, 0.8, 0.8,0.8);
		cairo_text_extents(ui->crf,"Tubeman", &extents);
		cairo_move_to (ui->crf, knobx1+knob_x/1.2-extents.width/1.6, knoby1+knob_y/1.8+extents.height/1.4);
		cairo_show_text(ui->crf, "Tubeman");
		cairo_new_path (ui->crf);
	}
...
static void bypass_expose(gx_cabsimUI *ui, gx_controller* switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void draw_controller(gx_cabsimUI *ui, gx_controller* controller) {
...
static void _expose(gx_cabsimUI *ui) {
	const char* plug_name = "GxCabSim" ;
...
static void controller_expose(gx_cabsimUI *ui, gx_controller * control) {
	cairo_push_group (ui->cr);
	cairo_scale (ui->cr, ui->rescale.x, ui->rescale.y);
...
static void send_controller_event(gx_cabsimUI *ui, int controller) {
...
static void check_value_changed(gx_cabsimUI *ui, int i, float* value) {
...
static void check_is_active(gx_cabsimUI *ui, int i, bool set) {
...
static bool aligned(int x, int y, gx_controller *control, gx_cabsimUI *ui) {
...
static bool get_active_ctl_num(gx_cabsimUI *ui, int *num) {
	bool ret = false;
	for (int i=0;i<CONTROLS;i++) {
...
static bool get_active_controller_num(gx_cabsimUI *ui, int *num) {
	for (int i=0;i<CONTROLS;i++) {
...
static void scroll_event(gx_cabsimUI *ui, int direction) {
	float value;
	int num;
...
static void enum_event(gx_cabsimUI *ui, int i) {
	float value;
...
static void motion_event(gx_cabsimUI *ui, double start_value, int m_y) {
...
static void set_key_value(gx_cabsimUI *ui, int set_value) {
	float value = 0.0;
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void key_event(gx_cabsimUI *ui, int direction) {
	float value;
	int num;
	if (get_active_controller_num(ui, &num)) {
		value = min(ui->controls[num].adj.max_value,max(ui->controls[num].adj.min_value, 
...
static void set_previous_controller_active(gx_cabsimUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void set_next_controller_active(gx_cabsimUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
		ui->controls[num].is_active = false;
		send_controller_event(ui, num);
...
static void get_last_active_controller(gx_cabsimUI *ui, bool set) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void event_handler(gx_cabsimUI *ui) {
	XEvent xev;
