...
#define MK_IMP(i) impulse2freq(c, i, IMP_LENGTH(i), impulse_freq[c]); c++; LOGD ("loaded impulse %d",c );
...
#endif
{
  ...
  unsigned int i, fftl = 128;
	LOGD("fftl: %d, length: %d, SEG_LENGTH: %d", fftl, length, SEG_LENGTH);
...
#ifdef FFTW3	
  LOGD("rc");
  plan_rc[id] = fftwf_plan_r2r_1d(fftl, real_in, comp_out, FFTW_R2HC, FFTW_ESTIMATE);
  LOGD("cr");
  plan_cr[id] = fftwf_plan_r2r_1d(fftl, comp_in, real_out, FFTW_HC2R, FFTW_ESTIMATE);
  LOGD("tmp");
  tmp_plan = fftwf_plan_r2r_1d(fftl, impulse_time, out, FFTW_R2HC, FFTW_ESTIMATE);
#else
  ...
  LOGD("setting impulse times");
  for (i=0; i<length; i++) {
    impulse_time[i] = imp[i];
  }
          
  LOGD("0ing other impulse times");
  int last = i;
  for (i = 0; i<fftl; i++) {
...
static void connectPortImp(
 ...
 LADSPA_Data *data) {
	...
	case FILE_BUFFER_SIZE:
		plugin->file_buffer_size = * data ;
		LOGD("set file buffer size %f", *data);
		break ;
	case FILE_BUFFER:
		LOGD("setting file buffer");
		plugin->file_buffer = (float *) malloc(plugin->file_buffer_size);
		for (int i = 0 ; i < plugin->file_buffer_size; i ++)
			plugin->file_buffer [i] = data [i];
		LOGD("creating freq data");
		impulse2freq (21, plugin->file_buffer, plugin->file_buffer_size, plugin->impulse_freq [21]);
		*plugin->impulse = 21;
		break ;
	}
}
